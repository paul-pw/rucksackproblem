...
\item Berechnune die Teilmengen:\\
  \textbf{for} j in range(1,n):\\
  ...
  \hspace*{15mm}\(s(j,k)=1\)\\
\item Wähle beste zulässige Teilmenge:\\
  \(k=\max\{i \in \{0,...,C\}:x(n,i)<\infty\}\)\\
  S=\{\}\\
...
Dieser Algorithmus hat eine Laufzeit von O(nW). Durch den Trick \(C:=\sum_{j\in S}^nc_j\) zu setzten erhält man eine Laufzeit von O(nC). Diese O(nC) Schranke ist nicht polynomiell in der Eingabegröße, jedoch erweist sich dieser Algorithmus in der Praxis, für kleinere Zahlen, als effizient.
